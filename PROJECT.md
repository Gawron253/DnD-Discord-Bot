# Project: D&D AI Discord Bot (Pure Discord Architecture + Gemini 2.0 Flash)
...
- **AI Dungeon Master (Google Gemini 2.0 Flash)**: Invoked **strictly on demand** via `@Mistrz Gry` mention or `/next` slash command on `#stół-gry`. Performs stateless history scanning (`after=last_bot_message`), synthesizes player declarations with dice results and live rules from `#zasady-i-mechanika`, and yields narrative responses with dynamic action buttons.
...
| F10 | Google Gemini 2.0 Flash Integration | `google-genai` / REST client with 4-layer hierarchical prompt assembly, custom RPG safety thresholds, and mock fallback | M3 | ORIGINAL_REQUEST §R3 | DONE |
...
- **Total Test Suite:** 314 Tests (100% Passing in 1.19s)
...
- **Unit & Integration Tests:** 126 Tests
